...
export function WithdrawalsTable() {
  const { t } = useLanguage()
  const { data: withdrawals, isLoading, processWithdrawal, rejectWithdrawal } = useWithdrawals()
  ...
  const columns: ColumnDef<any>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <Badge
            className={status === "To'langan" ? "bg-green-500" : status === "Kutilmoqda" ? "bg-gray-500" : "bg-red-500"}
          >
            {status}
          </Badge>
        )
          ...
          <div className="flex items-center">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                {isProcessable && (
                  <>
                    <DropdownMenuItem onClick={() => processWithdrawal(withdrawal.id)}>{t("process")}</DropdownMenuItem>
                    <DropdownMenuItem
    ...
    <div>
      <div className="flex flex-col md:flex-row gap-4 mb-4">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" onClick={clearFilters} disabled={!searchTerm && !statusFilter}>
      ...
      <div className="flex items-center justify-end space-x-2 py-4">
        <div className="flex-1 text-sm text-muted-foreground">
          {t("showing")} {filteredData.length > 0 ? currentPage * pageSize + 1 : 0} -{" "}
          {Math.min((currentPage + 1) * pageSize, filteredData.length)} {t("of")} {filteredData.length}
        </div>
        <Button
          variant="outline"
          size="sm"
        ...
        <Button
          variant="outline"
          size="sm"
          onClick={() => setCurrentPage((prev) => Math.min(Math.ceil(filteredData.length / pageSize) - 1, prev + 1))}
      ...
      <Dialog open={isRejectDialogOpen} onOpenChange={setIsRejectDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={handleRejectSubmit} disabled={!rejectReason}>
              {t("reject")}
            </Button>
          </DialogFooter>
        </DialogContent>
          ...
          {currentWithdrawal && (
            <div className="grid gap-4 py-4">
              <div className="grid grid-cols-2 gap-2">
                ...
                {currentWithdrawal.status === "Rad etilgan" && currentWithdrawal.reason && (
                  <>
