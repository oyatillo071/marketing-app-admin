...
export const exportToPDF = (
  ...
  const tableRows = data.map((item) => {
    return columns.map((col) => {
      ...
      const value =
        typeof item[col.accessor] === "object" ? JSON.stringify(item[col.accessor]) : String(item[col.accessor] || "")
      return value
    })
  })
...
export const exportStatsToPDF = (stats: any, title: string, fileName: string) => {
  const doc = new jsPDF()
